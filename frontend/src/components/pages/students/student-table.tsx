// LOKASI: src/components/pages/students/student-table.tsx
"use client";
...
export function StudentTable({
  students, isLoading, page, limit, totalItems, totalPages,
  activeTab, selectedIds, activatingId,
  onSelectAll, onSelectOne, onPageChange, onLimitChange,
  onVerify, onDetail, onEdit, onDelete,
}: StudentTableProps) {
  
  // Mencegah cache browser menampilkan foto lama
  const renderTime = useMemo(() => Date.now(), [students]);

  const getInstitutionName = (student: User) => {
    if ((student as any).institution_name) return (student as any).institution_name;
    if (student.enrollments && student.enrollments.length > 0) {
      return student.enrollments[0].institution?.name || "Lembaga Pendidikan";
    }
    return (student as any).institution?.name || "Lembaga Belum Diatur";
  };
        ...
        <Table>
          <TableHeader className="bg-slate-50 border-b border-slate-100">
            <TableRow>
              ...
              <TableHead className="text-[11px] uppercase font-bold text-slate-500">Profil Siswa</TableHead>
              <TableHead className="text-[11px] uppercase font-bold text-slate-500">NISN / NIK</TableHead>
              <TableHead className="text-[11px] uppercase font-bold text-slate-500">Kelas & Lembaga</TableHead>
              ...
              <TableRow>
                <TableCell colSpan={6} className="px-6 py-20 text-center text-slate-500 hover:bg-white">
                  <div className="flex flex-col items-center justify-center space-y-3">
                    ...
                    <p className="text-sm font-medium">Tidak ada data siswa yang ditemukan.</p>
                  </div>
                </TableCell>
              </TableRow>
            ) : (
              students.map((item: User, idx: number) => {
                const rawPhotoPath = (item.profile as any)?.image || (item.profile as any)?.photo;
                ...
                return (
                  <TableRow key={item.id} className="hover:bg-slate-50/80 transition-colors group">
                    <TableCell className="pl-6 text-center">
                      <Checkbox checked={selectedIds.includes(item.id)} onCheckedChange={(c) => onSelectOne(!!c, item.id)} />
                    </TableCell>
                      ...
                      <div className="flex items-center gap-3">
                        <Avatar className="h-10 w-10 border border-slate-200 shadow-sm relative">
                          ...
                          <AvatarFallback className="text-xs bg-emerald-50 text-emerald-700 font-bold">
                            {(item.profile?.full_name || "S").substring(0, 2).toUpperCase()}
                          </AvatarFallback>
                          {!item.is_active && (
                        ...
                        <div className="flex flex-col">
                          <span className="font-bold text-sm text-slate-800 cursor-pointer hover:text-emerald-700 transition-colors line-clamp-1" onClick={() => onDetail(item)}>
                    ...
                    <TableCell>
                      <div className="flex flex-col gap-1 text-[11px] font-mono">
                        <span className="bg-slate-100 text-slate-700 px-2 py-0.5 rounded border border-slate-200 w-fit font-semibold">
                          NISN: {item.profile?.nisn || "-"}
                        </span>
                        <span className="text-slate-500 pl-1">NIK: {item.profile?.nik || "-"}</span>
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="flex flex-col gap-1.5">
                        <div className="flex items-center gap-2 text-xs text-slate-700">
                          ...
                          <span className="font-semibold line-clamp-1">Kelas {item.profile?.class?.name || "Belum Ditentukan"}</span>
                        ...
                        <div className="flex items-center gap-2 text-[10px] text-slate-500 ml-5">
                           ...
                           <span className="truncate">{getInstitutionName(item)}</span>
                        </div>
                    ...
                    <TableCell className="text-right pr-6">
                      <div className="flex items-center justify-end gap-1.5 opacity-80 group-hover:opacity-100 transition-opacity">
                        ...
                        <Button variant="ghost" size="icon" onClick={() => onDetail(item)} className="h-8 w-8 text-blue-600 hover:text-blue-700 hover:bg-blue-50" title="Detail"><Eye className="h-4 w-4" /></Button>
                        <Button variant="ghost" size="icon" onClick={() => onEdit(item)} className="h-8 w-8 text-emerald-600 hover:text-emerald-700 hover:bg-emerald-50" title="Edit"><Pencil className="h-4 w-4" /></Button>
                        <Button variant="ghost" size="icon" onClick={() => onDelete(item.id)} className="h-8 w-8 text-rose-500 hover:text-rose-600 hover:bg-rose-50" title="Hapus"><Trash2 className="h-4 w-4" /></Button>
                      </div>
                    </TableCell>
                  </TableRow>
                );
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between px-6 py-4 border-t border-slate-100 bg-slate-50/50 gap-4 rounded-b-xl">
          <div className="flex items-center gap-3">
            <div className="text-xs text-slate-500">
              Menampilkan <span className="font-bold text-slate-800">{students.length}</span> dari <span className="font-bold text-slate-800">{totalItems}</span> data
            </div>
