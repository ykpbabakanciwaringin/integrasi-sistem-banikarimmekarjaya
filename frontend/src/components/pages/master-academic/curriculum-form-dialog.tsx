// LOKASI: src/components/pages/master-academic/curriculum-form-dialog.tsx
"use client";
...
import { Loader2, BookOpen, Building2, Pencil } from "lucide-react";
...
export function CurriculumFormDialog({
  ...
  const [formData, setFormData] = useState<Partial<Curriculum>>({
    name: "", is_active: true, institution_id: "",
  });
  ...
  useEffect(() => {
    if (open) {
      ...
      setFormData({ name: "", is_active: true, institution_id: "" });
    }
  }, [initialData, open]);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent aria-describedby={undefined} className="sm:max-w-[450px] p-0 overflow-hidden rounded-2xl border-0 bg-white shadow-2xl">
        <VisuallyHidden.Root><DialogTitle>Form Kurikulum</DialogTitle></VisuallyHidden.Root>
        ...
        <DialogHeader className="p-6 bg-[#043425] text-white shrink-0 relative overflow-hidden">
          ...
          <div className="flex items-center gap-4 relative z-10">
            <div className="h-12 w-12 bg-white/10 rounded-xl flex items-center justify-center border border-white/20 shadow-inner">
              {isEditMode ? <Pencil className="h-6 w-6 text-emerald-400" /> : <BookOpen className="h-6 w-6 text-emerald-400" />}
            </div>
            <div className="text-left space-y-1">
              <DialogTitle className="text-xl font-bold tracking-tight text-white">
                {isEditMode ? "Edit Kurikulum" : "Buat Kurikulum"}
              </DialogTitle>
              <DialogDescription className="text-emerald-100/80 text-xs">
                Struktur dasar mata pelajaran untuk lembaga.
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
        ...
        <div className="p-6 space-y-5 bg-slate-50/50">
          {isSuperAdmin && (
            <div className="space-y-2 bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
              ...
              <Select value={formData.institution_id} onValueChange={(val) => setFormData({ ...formData, institution_id: val })}>
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              className="h-10 bg-white border-slate-200 focus-visible:ring-emerald-500 shadow-sm transition-colors"
            />
          </div>

          <div className="flex flex-row items-center justify-between rounded-xl border border-slate-200 p-4 bg-white shadow-sm hover:border-emerald-200 transition-colors">
            <div className="space-y-0.5">
              <Label className="text-sm font-bold text-slate-700">Status Aktif</Label>
              <p className="text-[10px] text-slate-500 italic">Aktifkan untuk dapat digunakan di kelompok mapel.</p>
            </div>
            <Switch checked={formData.is_active} onCheckedChange={(c) => setFormData({ ...formData, is_active: c })} className="data-[state=checked]:bg-emerald-600" />
          </div>
        </div>
        ...
        <DialogFooter className="p-4 bg-slate-50 border-t border-slate-100 flex items-center justify-between shrink-0 rounded-b-2xl">
          ...
          <Button
            className="bg-emerald-600 hover:bg-emerald-700 text-white shadow-lg shadow-emerald-600/20 active:scale-95 transition-all font-bold px-6"
            onClick={() => onSubmit(formData)}
            disabled={isLoading || !formData.name || (isSuperAdmin && !formData.institution_id)}
          >
            {isLoading ? <Loader2 className="animate-spin mr-2 h-4 w-4" /> : "Simpan Kurikulum"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
