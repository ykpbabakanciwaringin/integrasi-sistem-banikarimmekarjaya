// LOKASI: src/components/pages/master-academic/holiday-data-table.tsx
"use client";
...
export function HolidayDataTable({ isSuperAdmin, userInstId, localFilterInstId, selectedMonth, institutions, onEdit }: HolidayDataTableProps) {
    ...
    <Card className="border-slate-200 shadow-sm overflow-hidden bg-white rounded-2xl relative">
      ...
      <div className="p-4 border-b border-slate-100 flex items-center justify-between gap-4 bg-slate-50/50">
         <div className="relative w-full max-w-sm">
            ...
            <Input placeholder="Cari keterangan libur insidental..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9 h-10 bg-white transition-colors border-slate-200 shadow-sm focus-visible:ring-emerald-500 rounded-xl text-xs font-semibold" />
         </div>
      </div>
      ...
      <div className="overflow-x-auto min-h-[300px]">
        <Table>
          ...
          <TableBody>
            {isLoading ? (
               <TableRow><TableCell colSpan={4} className="h-48 text-center"><Loader2 className="h-8 w-8 animate-spin mx-auto text-emerald-500"/></TableCell></TableRow>
              ...
              paginatedHolidays.map((item) => (
                <TableRow key={item.id} className="hover:bg-slate-50/50 transition-colors group">
                  ...
                  <TableCell className="font-bold text-slate-800 py-3 text-sm">{item.name}</TableCell>
              ...
              <TableRow><TableCell colSpan={4} className="h-48 text-center text-slate-400 font-medium">
                <div className="flex flex-col items-center"><CalendarOff className="h-12 w-12 opacity-20 mb-3" />Tidak ada jadwal libur.</div>
              </TableCell></TableRow>
            )}
          </TableBody>
        </Table>
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(v) => !v && setDeleteId(null)}>
        <AlertDialogContent className="rounded-2xl border-0">
          <AlertDialogHeader>
            <AlertDialogTitle className="font-bold flex items-center gap-2 text-rose-600"><Trash2 className="h-5 w-5" /> Hapus Libur?</AlertDialogTitle>
            <AlertDialogDescription>Tindakan ini bersifat permanen.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel className="rounded-xl">Batal</AlertDialogCancel>
            <AlertDialogAction className="bg-rose-600 hover:bg-rose-700 text-white rounded-xl" onClick={() => deleteId && deleteMutation.mutate(deleteId)}>
              {deleteMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : null} Ya, Hapus
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
