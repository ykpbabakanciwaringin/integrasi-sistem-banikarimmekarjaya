// LOKASI: src/components/pages/manage-exams/[eventId]/sessions/use-participant-print.ts
...
const convertImageToBase64 = async (url: string): Promise<string | null> => {
  ...
  try {
    ...
    // Perubahan ada pada baris di bawah ini: ditambahkan tipe <string | null>
...
export function useParticipantPrint(selectedIds: string[], onClearSelection: () => void) {
  ...
  const startPrintProcess = async (sessionId: string, mode: "single" | "a4") => {
    ...
    try {
      // 1. Ambil data rahasia dari server
      const response = await examService.downloadExamCards(sessionId);
      const rawData = response?.data || response || [];
      
      if (!rawData || rawData.length === 0) throw new Error("Data kartu tidak ditemukan di server.");

      const targets = rawData.filter((p: any) => selectedIds.includes(p.student_id));
      toast.loading(`Menyiapkan aset visual untuk ${targets.length} peserta...`, { id: toastId });

      const baseUrl = typeof window !== 'undefined' ? window.location.origin : '';
      
      // ---  OPTIMASI: SISTEM CACHE LOGO ---
      // Kita hanya download logo satu kali untuk semua kartu
      ...
      const printItems = await Promise.all(
        targets.map(async (p: any) => {
          // Foto diproses per siswa (karena unik)
          ...
          return {
            ...
            logoUrl: cachedLogoB64, // Menggunakan cache
            photoUrl: photoB64,
            qrCodeUrl: qrDataUrl, 
        ...
        for (let i = 0; i < printItems.length; i++) {
          ...
          if (singlePrintRef.current) {
            const dataUrl = await toPng(singlePrintRef.current, { 
              pixelRatio: 3, // Keseimbangan antara tajam vs ukuran file
              cacheBust: true,
            });
      ...
      // =====================================
        ...
        for (let i = 0; i < printItems.length; i++) {
          ...
          if (singlePrintRef.current) {
            const dataUrl = await toPng(singlePrintRef.current, { 
              pixelRatio: 2.5, // Cukup tajam untuk PDF tanpa lag
              cacheBust: true 
            });
