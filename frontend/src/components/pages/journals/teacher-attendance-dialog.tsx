// LOKASI: src/components/pages/journals/teacher-attendance-dialog.tsx
"use client";
...
import { Loader2, Users, CheckCircle2, MessageSquareHeart, CloudUpload, CloudOff, UserCheck, Lock, RefreshCw } from "lucide-react";
...
interface TeacherAttendanceDialogProps {
  ...
  journal?: any | null;
  institutionId?: string; // Tambahan agar sesuai dengan pemanggilan di history-tab
}

export function TeacherAttendanceDialog({ open, onOpenChange, journal, institutionId }: TeacherAttendanceDialogProps) {
  ...
  const journalDate = journal?.date ? dayjs(journal.date).startOf('day') : dayjs().startOf('day');
  ...
  //  DETEKSI: Apakah ada data yang gagal tersinkronisasi?
  const hasSyncErrors = Object.values(attendanceRecords).some(
    (record) => record.sync_error && !record.synced
  );
  ...
  const retrySyncMutation = useMutation({
    mutationFn: () => journalService.retrySyncAttendance(journalId),
    onSuccess: (res: any) => {
      toast.success(res?.message || "Perintah sinkronisasi ulang sedang diproses...");
      queryClient.invalidateQueries({ queryKey: ["journals"] });
      queryClient.invalidateQueries({ queryKey: ["attendances", journalId] });
    },
    onError: (err: any) => {
      toast.error(err?.response?.data?.error || err.message || "Gagal melakukan sinkronisasi ulang.");
    },
  });
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl p-0 overflow-hidden bg-slate-50 flex flex-col max-h-[90vh]">
        <DialogHeader className="p-6 pb-4 bg-white border-b border-slate-100 shrink-0">
          <div className="flex items-center justify-between">
            <div>
              <DialogTitle className="text-xl flex items-center gap-2">
                <Users className="h-6 w-6 text-indigo-600" /> {isLocked ? "Detail Kehadiran Kelas (Piket)" : "Pengisian Absensi Kelas (Piket)"}
              </DialogTitle>
              <DialogDescription className="mt-1">
                {journal?.allocation?.subject?.name || "Pilih Jurnal untuk mulai"} — {journal?.allocation?.class?.name}
              </DialogDescription>
            </div>
          ...
          {hasSyncErrors && (
            <div className="mt-4 p-3 bg-rose-50 border border-rose-200 rounded-lg flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-sm">
              <div className="flex items-center text-rose-800 text-xs font-semibold">
                <CloudOff className="w-5 h-5 mr-2 shrink-0 text-rose-600" />
                Sebagian data absensi gagal disinkronkan ke server PesantrenQu.
              </div>
              <Button 
                size="sm" 
                variant="outline"
                className="w-full sm:w-auto bg-white border-rose-300 text-rose-700 hover:bg-rose-100 hover:text-rose-800"
                onClick={() => retrySyncMutation.mutate()}
                disabled={retrySyncMutation.isPending || isLocked}
              >
                {retrySyncMutation.isPending ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <RefreshCw className="w-4 h-4 mr-2" />}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 relative">
          {!journalId ? (
            <div className="py-20 text-center text-slate-400">Silakan pilih jurnal dari riwayat untuk mengelola absensi.</div>
          ) : isLoading ? (
